...
if __name__ == "__main__":
    for item in tqdm(names):
        icm = cv2.imread(os.path.join(GT_ICM, item.replace(
            ".BMP", " ICM_Mask.bmp")), cv2.IMREAD_GRAYSCALE)
        te = cv2.imread(os.path.join(GT_TE, item.replace(
            ".BMP", " TE_Mask.bmp")), cv2.IMREAD_GRAYSCALE)
        zp = cv2.imread(os.path.join(GT_ZP, item.replace(
            ".BMP", " zp_Mask.bmp")), cv2.IMREAD_GRAYSCALE)
        size = icm.shape
